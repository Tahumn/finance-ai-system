...
s_marker = b"def answer_chat_REPLACED_TEST("
if s_marker not in source:
    s_marker = b"def answer_chat("

s = source.find(s_marker)
e = source.find(b"def _otsu_threshold(")

if s == -1 or e == -1:
    print(f"FAILED: Markers not found {s} {e}")
    exit(1)
...
new_source = source[:s] + new_logic.encode("utf-8") + b"\n\n" + source[e:]
