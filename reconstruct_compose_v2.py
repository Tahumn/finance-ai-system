...
# Update API service to depend on redis
if 'redis' not in compose['services']['api'].get('depends_on', {}):
    if isinstance(compose['services']['api'].get('depends_on'), list):
        compose['services']['api']['depends_on'].append('redis')
    elif isinstance(compose['services']['api'].get('depends_on'), dict):
        compose['services']['api']['depends_on']['redis'] = {'condition': 'service_started'}

# Add Postgres init script mapping
...
# Add Kafka (Updated tag to 3.7 to avoid pull issue)
...
if 'redis_data' not in compose['volumes']:
