...
### *A AI-Native platform with NLP Automation & Real-time Analytics*
...
3. [Các phân hệ tính năng cốt lõi](#-các-phân-hệ-tính-năng-cốt-lõi)
...
9. [Minh họa sản phẩm (Showcase)](#-minh-họa-sản-phẩm-showcase)
...
*   **Tự động hóa toàn diện:** Sử dụng kiến trúc hiện đại và hàng đợi thông minh để xử lý mọi tác vụ nặng dưới nền, mang lại trải nghiệm người dùng liền mạch.
...
Hệ thống được thiết kế theo kiến trúc **AI-Native Architecture**, kết hợp giữa sức mạnh của FastAPI và hệ sinh thái AI Orchestration để tối ưu hóa khả năng xử lý thông minh.

### 1. Thành phần cốt lõi (Core Components)
Hệ thống bao gồm các thành phần cốt lõi hoạt động nhịp nhàng:
*   **API Engine (FastAPI):** Đóng vai trò là trung tâm điều khiển, xử lý logic nghiệp vụ, quản lý cơ sở dữ liệu và cung cấp API RESTful.
*   **AI Orchestrator (Dify.AI):** Quản lý các Agent, xử lý hội thoại thông minh và tích hợp các mô hình LLM (Gemini 1.5).
*   **Automation Hub (n8n):** Xử lý các quy trình tự động hóa phức tạp, gửi thông báo và kết nối các dịch vụ ngoại vi.
*   **Data Store (PostgreSQL):** Lưu trữ dữ liệu giao dịch với tính toàn vẹn cao.

### 2. Giao tiếp & Luồng dữ liệu
...
*   **Asynchronous (Background Tasks):** Sử dụng hàng đợi công việc cho các tác vụ tốn thời gian như Xử lý OCR phức tạp và Phân tích báo cáo chuyên sâu.
...
*   **Real-time Balance:** Số dư được cập nhật tức thời ngay khi có biến động, loại bỏ hoàn toàn độ trễ dữ liệu.
...
Sử dụng các kỹ thuật xử lý ảnh để thực hiện:
...
Đây là bước quan trọng nhất – dữ liệu thô được đưa qua LLM để:
...
Toàn bộ quá trình xử lý ảnh được xử lý dưới nền, giúp người dùng có thể tiếp tục sử dụng ứng dụng ngay lập tức mà không cần chờ đợi.
...
| **Database** | PostgreSQL 16 | Chuẩn mực cho tính toàn vẹn dữ liệu tài chính. |
| **Automation** | n8n | Linh hoạt trong việc thiết lập các workflow tự động. |
...
*   **Phân quyền:** Hệ thống phân quyền dựa trên Role (RBAC) đảm bảo an toàn dữ liệu.
*   **Tối ưu hóa:** Caching dữ liệu thường xuyên truy cập để giảm tải cho PostgreSQL.
*   **Hạ tầng:** Dockerized toàn bộ hệ thống, giúp triển khai nhất quán trên mọi môi trường.
