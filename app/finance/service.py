...
def update_category(
    db: Session, current_user: User, category_id: int, payload: schemas.CategoryUpdate
) -> Category:
    db_category = (
        db.query(Category)
...
def list_accounts(db: Session, current_user: User) -> list[Account]:
    return (
        db.query(Account)
...
def create_account(db: Session, current_user: User, payload: schemas.AccountCreate) -> Account:
...
def update_account(db: Session, current_user: User, account_id: int, payload: schemas.AccountUpdate) -> Account:
    item = (
        db.query(Account)
        .filter(Account.id == account_id, Account.user_id == current_user.id)
        .first()
...
def list_savings_goals(db: Session, current_user: User) -> list[schemas.SavingsGoalRead]:
    rows = (
        db.query(SavingsGoal)
        .filter(SavingsGoal.user_id == current_user.id)
...
def delete_savings_goal(db: Session, current_user: User, goal_id: int) -> None:
    item = (
        db.query(SavingsGoal)
        .filter(SavingsGoal.id == goal_id, SavingsGoal.user_id == current_user.id)
        .first()
...
def bootstrap_finance_data(db: Session, current_user: User) -> dict:
